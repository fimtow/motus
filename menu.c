...
void votreScore(int score,int* stop,char mot[],int diff)
{
    ...
    while(!*stop)
    {
        ...
        SDL_SetRenderDrawColor(rend,241,196,15,255);
        SDL_RenderClear(rend);
        afficherText("le mot est :",0,30,50,255,255,255,255);
        afficherText(mot,0,85,50,255,255,255,255);
        afficherText("Votre score est :",0,150,60,255,255,255,255);
        char scores[10];
        sprintf(scores, "%d",score);
        afficherText(scores,0,225,60,255,255,255,255);
        if(ajouter)
        {
            afficherText("Entrer votre nom :",0,300,60,255,255,255,255);
            afficherText(nom,0,380,60,255,255,255,255);
        }
        SDL_RenderPresent(rend);
    }
}
...
void menuOptions(options* mesOptions,int* stop)
{
    ...
    while(!*stop)
    {
        ...
        SDL_SetRenderDrawColor(rend,241,196,15,255);
        SDL_RenderClear(rend);
        afficherText("Options",0,30,50,255,255,255,255);
        afficherText("Difficulté",10,145,40,255,255,255,255);
        afficherParametres(difficulte,3,10,&t[0],335,140,170,50,clique);
        afficherText("Nbr de lettres",10,205,40,255,255,255,255);
        afficherParametres(nbrLettres,5,3,&t[1],335,200,170,50,clique);
        afficherText("temps",10,265,40,255,255,255,255);
        afficherParametres(tempsReflexion,3,3,&t[3],335,260,170,50,clique);
        afficherText("Sons",10,325,40,255,255,255,255);
...
void menuHighscore(int* stop,int diff)
{
    ...
    while(!*stop)
    {
        ...
        SDL_SetRenderDrawColor(rend,241,196,15,255);
        SDL_RenderClear(rend);
        afficherText("HighScores :",0,10,50,255,255,255,255);
        int i=0;
        while(tableau[i].sc != -1 && i<7)
        {
            afficherText(tableau[i].nom,30,100+50*i,40,255,255,255,255);
            char score[10];
            sprintf(score, "%d",tableau[i].sc);
            afficherText(score,450,100+50*i,40,255,255,255,255);
            i++;
        }
        SDL_RenderPresent(rend);
        SDL_Delay(1000/60);
